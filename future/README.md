...
> **活文档。** 随研究推进更新。状态：**EXP1 / EXP2 主体完成**，但见下方重要修正。

> ## ⚠️ 重要修正（2026-06，来自 `../experiment/` 跨合约×分年度检验）
> 本目录早期的两个"亮点"**已被证伪/打折**，读下面正文时请带上这个滤镜：
> 1. **"大脉冲 → −2 点超价差反转"基本不成立。** 这里的坏 tick 过滤只做了 despike（去单点尖峰），**没在源头滤掉缺价快照**（bid 或 ask=0 → mid 被腰斩的假跳）。`experiment/` 在取数源头加 `bid>0 & ask>0` 后，IM `n=10,k=6点` 从 **−2.09 点**变成 **+0.19 点** → 那个 −2 点**大半是坏 tick 假反转**。**正确做法 = 源头滤波**（见 `experiment/exp1_cross_ddb.py`）。
> 2. **"大脉冲反转"不稳定，逐年翻号。** 跨 IC/IF/IH/IM、2015-2026 分年度看：大脉冲反转只集中在**少数危机年**（IF/IH 的 2016-17 股灾熔断、IM 的 2022-23），**多数年份大脉冲其实是趋势（延续）**。这是 regime/过拟合，**不是稳定可交易结构**。
> 3. **唯一稳的是"小脉冲微动量"**（小而快的脉冲延续，跨年跨合约稳健，但 ≤0.1 点、在价差内）。
> 详见 [`../experiment/README.md`](../experiment/README.md)。下文 EXP1 关于"超价差反转格"的描述以此修正为准。
...
2. **越大的脉冲 → 越深的反转**（仅在**池化 + 仅 despike** 下成立）。⚠️ **已被修正**：`n=10,k=6点 → −2.09` 那类"超价差反转"在**源头滤坏tick（bid/ask>0）后塌成 +0.19**、且**逐年翻号**——大半是坏 tick + 危机年专属，**非稳定可交易**（见顶部修正与 `../experiment/`）。
