...
| 5 | **Cont, Cucuringu & Zhang (2023)**, *Cross-Impact of Order Flow Imbalance*, Quant. Finance | 一个合约的订单流能否预测**别的**合约的价格？（跨合约领先-滞后 + 跨OFI回归） | [STUDY_GUIDE_CROSSIMPACT.md](STUDY_GUIDE_CROSSIMPACT.md) | `crossimpact_ddb.py` |
...
Cont-Cucuringu-Zhang cross-impact ([arXiv:2112.13213](https://arxiv.org/abs/2112.13213))。
...
| #5 跨合约 仅自身OFI | 样本外 R²（1s） | 0.015 | 0.019 | 0.017 | 0.018 |
| #5 跨合约 +他人OFI | 样本外 R²（1s） | **0.058** | **0.051** | **0.044** | **0.041** |
...
图：`fig61–65` OFI；`fig71–73` QI；`fig81–86` VOI + 回测；`fig91–93` 微观价格；`fig101–106` 跨合约(含IC↔IM深挖+回测+做市)。

**跨合约（论文5，`crossimpact_ddb.py`，图101–103）**：4 个合约按 W 秒对齐到同一时钟（W≥1s，0.5s 因与500ms采样混叠而退化）。
① 领先-滞后：6 对全部在 lag0 达峰 → **同步共动，无 500ms 以上的领先**。
② 跨OFI预测：`r_i(t+1) ~ 自身+他人 OFI(t)`，加入他人订单流把样本外 R²（1s）从 ~0.017 抬到 ~0.04–0.06（**翻 2–4 倍，且样本外稳定**）；系数矩阵显示**小盘 IC↔IM 互驱最强**（跨系数>自身）。
诚实解读：这更像**共同因子**（用整篮子的盘口更好地 nowcast 大盘的共同变动），而非"X 因果领先 Y"——要分离真正的定向跨冲击需先剥离共同因子（见指南5 §7 练习1）。

**小盘对 IC↔IM 深挖 + 做市资本收官（图104-106）**：
- 图104 `xc_icim_ddb`：配对 `r_IC(t+1)~OFI_IC+OFI_IM` 样本外 R² 翻2.4倍且 OOS≈IS；控制 IF/IH 后跨系数仅从0.087→0.071，**~80%存活=真·小盘子因子**（非市场beta）；IM 微弱领先 IC。
- 图105 `xc_icim_backtest`：用 IM 交易 IC，信号价值 own 0.45→pair 0.72 tick（IM 真让信号更准），但跨 IC 宽价差后 taker 每笔 −6~−17 tick，**pair 亏得比 own 更多**（更准的信号→交易更多→亏更多）＝"signal≠alpha"最锋利的一课。
- 图106 `xc_maker`：**收官反转**。做市 IC（fills 由主动成交量推断，P&L=markout），三种报价中心 mid/P_micro/P_micro+跨漂移，样本外每笔 markout **2.07→2.21→2.95 tick**（cross比mid +0.88），信号逐层减少逆向选择。**杀死 taker 的宽价差正是 maker 的收入**。⚠毛收益、乐观假设（永远touch成交、mid离场、无手续费/排队），稳的是相对结论 mid<micro<cross，不是绝对值。项目主题由此钉死：**钱不在 taker 预测，在 maker 报价。**
...
crossimpact_ddb.py / crossimpact_plot.py         论文5 跨合约 (xc_*.csv, 图101-103)
xc_icim_ddb.py / xc_icim_plot.py                 IC↔IM 深挖 IS/OOS (图104)
xc_icim_backtest_ddb.py / _plot.py               IC↔IM taker 回测 (图105)
xc_maker_ddb.py / xc_maker_plot.py               IC 做市收官: P_micro+跨漂移 (图106)
...
- ~~跨合约 cross-impact~~ ✅ 已做（论文5，图101-103）：无领先-滞后，跨OFI预测翻2-4倍但像共同因子。
- **剥离共同因子**：对篮子收益回归后取残差，再测跨OFI——分离真·定向跨冲击 vs 共同因子（指南5 §7）。
- 把 `P_micro` 代回 intraminute/ticker 的旧研究当"更准的尺子"（指南4 §6）。
- maker 版回测：挂单在己方最优价、用后续快照判断成交——检验"做市方向"能否翻正。
- **方向B 价格冲击**：Kyle's λ + 平方根律——量化"信号在价差内"的成本曲线（Kyle 1985；Tóth 2011）。
- **方向C 波动率**：HAR-RV + 跳跃——预测波动比预测方向容易且真能用（Corsi 2009）。
