"""tick2min_ddb.py — exemplar: aggregate hft_future_ts 500ms ticks to 1-minute bars.

Two lessons in one file:
  A) Aggregate SERVER-SIDE in DolphinDB SQL (`group by bar(ts, 60s)`) — ship ~330
     minute rows/day instead of ~25k tick rows/day. This is the way for anything
     spanning months.
  B) Validate against a pandas resample of the raw ticks for one day — never trust
     an aggregation you haven't cross-checked once.

Field semantics (probed 2026-07, IF0000 2024-06-03):
  - m_iVolume / m_iTurnover are PER-SNAPSHOT INCREMENTS (not cumulative);
    m_iAccVolume / m_iAccTurnover are the day-cumulative versions.
  - We build minute volume from last(acc) DIFFED per day, NOT sum(increments):
    sum() double-counts the 2024-02 duplicate-row months; last()+diff is immune.
  - 09:29 carries the opening-auction snapshot (kept as diff baseline, then dropped);
    stray rows can appear after 15:00 (filtered); the 11:30:00 / 15:00:00 close
    snapshots land in their own bar label and are folded back into 11:29 / 14:59.
  - LIMIT-UP quirk (IF 2024-09-30, 2024-10-08): ask side empties -> m_nAskPrice=0
    -> raw mid halves ("half-price quote"). pb/pa therefore use lastNot(iif(...)):
    last VALID quote of the minute (pb>0, pa>pb, spread<=10 pts); a fully one-sided
    minute gets pb/pa=NaN and mid_close=NaN. OHLC/volume (trade-based) unaffected.

Output: min_bars_<code>.csv with ts(bar start), OHLC (last-trade px), vol(lots),
amt(yuan), vwap, nticks, last quotes pb/pa, mid_close.
Run:  PILOT=1 /Users/zhuisabella/xn/.venv/bin/python tick2min_ddb.py   (sandbox OFF)
"""
import os
import sys
import numpy as np
...
# ---------- A) server-side minute aggregation ----------------------------------
def fetch_min_bars(sess, code, start, end):
    """One SQL round-trip: floor每条快照到分钟 (bar), 组内聚合. first/last/max/min
    are duplicate-row-safe; count(*) is not (kept only as a diagnostic)."""
    q = f"""
    ...
    select first(m_nPrice)    as open,
           ...
           lastNot(iif(m_nBidPrice>0 and m_nAskPrice>m_nBidPrice and m_nAskPrice-m_nBidPrice<=10.0, m_nBidPrice, 00F)) as pb,
           lastNot(iif(m_nBidPrice>0 and m_nAskPrice>m_nBidPrice and m_nAskPrice-m_nBidPrice<=10.0, m_nAskPrice, 00F)) as pa,
           count(*)           as nticks
    from pt
...
def to_session_bars(b, code):
    """SQL bars -> clean per-minute bars: fold close snapshots, diff acc fields,
    keep only the trading sessions."""
    b = b.copy()
    b["ts"] = pd.to_datetime(b["ts"])
    # fold the lone 11:30:00 / 15:00:00 close-snapshot bars into 11:29 / 14:59
    ...
    # per-day diff of cumulative fields; the 09:29 auction bar is the baseline,
    # so the 09:30 bar gets continuous-session volume only
    day = b.ts.dt.normalize()
    b["vol"] = b.groupby(day)["accvol"].diff()
    ...
    # keep the two continuous sessions (bar START in 09:30-11:29 / 13:00-14:59)
    hm = b.ts.dt.hour * 100 + b.ts.dt.minute
...
# ---------- B) one-day pandas cross-check ---------------------------------------
def check_one_day(sess, code, day, bars):
    """Rebuild the same bars from raw ticks in pandas; assert OHLC/vol agree."""
    q = f"""
    pt = loadTable("dfs://hft_future_ts","TickPartitioned")
