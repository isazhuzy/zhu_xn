...
H = 30
PX = "mid_close"   # quote midpoint; trade-price version uses "close"
...
for CODE in CODES:
    ...
    s["t"] = s["mean"] / s["se"]
    s.to_csv(f"{D}/fwd{H}_ret_{CODE}{SUF}.csv", index=False)
    ...
    ax.xaxis.set_minor_locator(AutoMinorLocator(3))   # gridlines
    ax.yaxis.set_minor_locator(AutoMinorLocator(2))
    ax.set_xlabel("日内分钟 t（信号 = 第 t 分钟方向）")
    ax.set_ylabel(f"未来{H}分钟平均收益（原始比例）")
    ax.set_title(f"{CODE} sign(第t分钟涨跌) × 未来{H}分钟平均收益（中间价，原始比例）"
                 f"（{START}–{END}）", fontsize=11, fontweight="bold")
    ax.legend()
    ax.grid(True, which="major", alpha=.35)
    ax.grid(True, which="minor", alpha=.12, lw=.5)
    fig.tight_layout(); fig.savefig(f"{D}/fig_fwd{H}_ret_{CODE}{SUF}.png", dpi=135)
    plt.close(fig)
    print(f"saved fwd{H}_ret_{CODE}{SUF}.csv + fig")
