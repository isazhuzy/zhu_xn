...
for CODE in CODES:
    ...
    s["t"] = s["mean"] / s["se"]
    s.to_csv(f"{D}/fwd{H}_ret_trade_{CODE}{SUF}.csv", index=False)
    ...
    fig.tight_layout(); fig.savefig(f"{D}/fig_fwd{H}_ret_trade_{CODE}{SUF}.png", dpi=135)
    plt.close(fig)
    print(f"saved fwd{H}_ret_trade_{CODE}{SUF}.csv + fig")
